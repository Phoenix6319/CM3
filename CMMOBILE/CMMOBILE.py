...
# Base de données Coupe du Monde avec Drapeaux
STATS_MONDE = {
    "🇦🇷 Argentine": {"gf": 2.20, "ga": 0.70}, "🇫🇷 France": {"gf": 2.35, "ga": 0.80},
    "🇧🇷 Brésil": {"gf": 2.10, "ga": 0.85}, "🏴󠁧󠁢󠁥󠁮󠁧󠁿 Angleterre": {"gf": 2.25, "ga": 0.75},
    "🇪🇸 Espagne": {"gf": 2.40, "ga": 0.65}, "🇩🇪 Allemagne": {"gf": 2.15, "ga": 1.10},
    "🇵🇹 Portugal": {"gf": 2.30, "ga": 0.90}, "🇮🇹 Italie": {"gf": 1.60, "ga": 0.80},
    "🇳🇱 Pays-Bas": {"gf": 1.95, "ga": 1.05}, "🇧🇪 Belgique": {"gf": 1.80, "ga": 1.15},
    "🇭🇷 Croatie": {"gf": 1.50, "ga": 0.95}, "🇲🇦 Maroc": {"gf": 1.75, "ga": 0.70},
    "🇸🇳 Sénégal": {"gf": 1.65, "ga": 0.85}, "🇺🇾 Uruguay": {"gf": 1.85, "ga": 1.00},
    "🇨🇴 Colombie": {"gf": 1.70, "ga": 0.90}, "🇺🇸 États-Unis": {"gf": 1.55, "ga": 1.20},
    "🇲🇽 Mexique": {"gf": 1.40, "ga": 1.25}, "🇯🇵 Japon": {"gf": 2.05, "ga": 0.95},
    "🇰🇷 Corée du Sud": {"gf": 1.60, "ga": 1.10}, "🇨🇭 Suisse": {"gf": 1.45, "ga": 1.15},
    "🇩🇰 Danemark": {"gf": 1.50, "ga": 1.05}, "🇳🇬 Nigeria": {"gf": 1.55, "ga": 1.10},
    "🇩🇿 Algérie": {"gf": 1.60, "ga": 1.00}, "🇹🇳 Tunisie": {"gf": 1.15, "ga": 0.90},
    "🇪🇬 Égypte": {"gf": 1.45, "ga": 0.95}, "🇨🇲 Cameroun": {"gf": 1.35, "ga": 1.20},
    "🇬🇭 Ghana": {"gf": 1.30, "ga": 1.35}, "🇨🇮 Côte d'Ivoire": {"gf": 1.65, "ga": 1.00},
    "🇪🇨 Équateur": {"gf": 1.40, "ga": 1.05}, "🇨🇦 Canada": {"gf": 1.45, "ga": 1.30},
    "🇦🇺 Australie": {"gf": 1.35, "ga": 1.15}, "🇸🇦 Arabie Saoudite": {"gf": 1.10, "ga": 1.40}
}
...
# Suppression de sorted() pour figer l'ordre de la base de données brute (Évite les bugs d'indexation)
liste_pays = list(STATS_MONDE.keys())

# Sélection simplifiée sans index forcé (Zéro blocage)
home_team = st.selectbox("Sélectionnez l'Équipe Domicile (A)", liste_pays)
away_team = st.selectbox("Sélectionnez l'Équipe Extérieur (B)", [p for p in liste_pays if p != home_team])

# Sécurité si les deux sélections se croisent temporairement lors du rechargement
if home_team == away_team:
    st.error("Veuillez sélectionner deux équipes différentes.")
else:
    home_lambda = STATS_MONDE[home_team]["gf"] * STATS_MONDE[away_team]["ga"] / 1.15
    away_lambda = STATS_MONDE[away_team]["gf"] * STATS_MONDE[home_team]["ga"] / 1.15
    p_home, p_draw, p_away, matrix = calculate_poisson_probabilities(home_lambda, away_lambda)

    # Calculs mi-temps et buts secondaires
